...
double computeSteepestPossibleSlopeToOther(int inX, int inY, double inValue, int quadX, int quadY, int depth, int quadLength) {
  ...
  double minQuadValue = getPixel(mins[depth], quadX, quadY, layerWidth); // mins[depth]->pixels[quadY][quadX] ; 
  double maxQuadValue =  getPixel(maxs[depth], quadX, quadY, layerWidth); // maxs[depth]->pixels[quadY][quadX] ;
  double largestDiff = fmax(inValue-minQuadValue, maxQuadValue-inValue); // Always zero or positive;
  double largestDiffSqr = largestDiff * largestDiff;
  double steepestPossibleSlopeSqr = largestDiffSqr / minDistanceSqr ; 
...
#pragma omp parallel for  collapse(2)
  for (int targetY=0 ; targetY<targetHeight ; targetY++) { 
    for (int targetX=0 ; targetX<targetWidth ; targetX++) {  /* PJ moved this up 2 lines - for better OMP! */
      ...
      double quadMin = getPixel(sourceMins, sourceXStart, sourceYStart, sourceWidth); 
      double quadMax = getPixel(sourceMaxs, sourceXStart, sourceYStart, sourceWidth); 
...
double searchWholeQuad(int inX, int inY, double inValue, int quadX, int quadY, int depth, double steepestYet) { 
  ...
  if (depth>0) { 
    ...
    double slopeSqr = valueDiff*valueDiff / distanceSqr ;
